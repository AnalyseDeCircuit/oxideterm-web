...
OxideTerm uses `russh 0.59` compiled against the **`ring`** crypto backend:
...
Not every important field belongs in the OS keychain, so OxideTerm also encrypts saved local configuration at rest.

- The on-disk local config is stored as an encrypted document, not plaintext JSON
- A dedicated local config key is kept in the OS keychain
- Connection metadata, jump routes, labels, and related saved state are sealed locally with ChaCha20-Poly1305
- Older plaintext configs are migrated to encrypted storage on startup

In practice: secrets stay in the system keychain, while the rest of your saved connection state is still protected on disk.
...
- Uses `~/.ssh/known_hosts` with OpenSSH-compatible behavior
...
- **Local-first by default** — no mandatory account, no required cloud service
- **Local-first** — bring your own API keys for AI features
- **Opt-in cloud workflows** — Cloud Sync is available as an official plugin, not a core requirement
