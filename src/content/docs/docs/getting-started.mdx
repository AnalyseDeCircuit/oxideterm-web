---
title: Introduction
description: OxideTerm — an AI-native workspace for remote servers built with pure Rust SSH and modern web technologies.
---

**OxideTerm** is an **AI-native workspace for remote servers**, not just a terminal. Connect to a server once, then work around it in one place: SSH shell, SFTP, port forwarding, trzsz transfers, lightweight editing, local shell, and OxideSens AI.
...
OxideTerm is not a cloud AI platform, hosted agent service, generic remote-protocol toolbox, or a terminal-rendering benchmark project. Many modern terminals are evolving around local shells, AI panels, or cloud agent platforms; OxideTerm focuses on local-first work on real remote servers over SSH.

## Why OxideTerm?

| Pain Point | OxideTerm's Answer |
|---|---|
| Remote server work is split across too many tools | **AI-native remote-server workspace**: terminal, SFTP, port forwarding, trzsz, lightweight IDE, monitoring, and OxideSens AI live under the same node |
...
| **SSH** | russh 0.61 (`ring`) | Pure Rust, zero OpenSSL, SSH Agent |
...
| **AI (OxideSens)** | Target-first BYOK AI. It sees saved connections, live SSH sessions, terminal buffers, SFTP paths, settings, and knowledge base entries as workspace targets; it can diagnose remote output, run approved commands, inspect files, and explain failures without requiring an OxideTerm account |
...
- [OxideSens AI](/docs/features/ai/) — target-first BYOK AI for remote-server work
