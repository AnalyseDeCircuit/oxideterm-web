---
title: Introduction
description: OxideTerm — an AI-native SSH workspace built with pure Rust and modern web technologies.
---

import { Image } from 'astro:assets';

**OxideTerm** is an AI-native SSH workspace for terminals, files, ports, and remote context. Built with **Tauri 2.0** (Rust backend) and **React 19** (TypeScript frontend), it delivers a lightweight 25–40 MB native binary with zero Electron, zero OpenSSL, zero telemetry, and pure Rust SSH.
...
| Category | Features |
|---|---|
| **Terminal** | Local PTY (zsh/bash/fish/pwsh/WSL2), SSH remote, split panes, broadcast input, session recording/playback (asciicast v2), adaptive rendering (120 Hz+ Boost / 60 Hz Normal / 1–15 Hz Idle), command palette (`⌘K`), zen mode, 31+ themes + custom editor |
...
| **Plugins** | Runtime ESM loading, 18 API namespaces, 24 UI Kit components, frozen API + Proxy ACL, circuit breaker, auto-disable on errors. Official plugins include Cloud Sync, Quick Commands, and Telnet Client |
