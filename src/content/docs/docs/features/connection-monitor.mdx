...
A connection that exceeds the idle timeout updates native node state. The GPUI workspace consumes that state change directly to refresh the sidebar, event log, and dependent node surfaces.
