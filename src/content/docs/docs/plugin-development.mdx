...
> Applies to the current OxideTerm release (Plugin API v3)

Current host-wired UI extension points include:

- `ctx.ui.registerContextMenu()` for `terminal`, `sftp`, `tab`, and `sidebar` targets
- `ctx.ui.registerStatusBarItem()` rendered in the main bottom status bar
- `ctx.ui.registerKeybinding()` dispatched by the global keybinding pipeline after built-ins and before terminal hook shortcuts
- `ctx.ui.showProgress()` rendered as a lightweight top-right progress HUD that auto-dismisses at 100%
...
Possible values: `"onConnect"` | `"onDisconnect"` | `"onReconnect"` | `"onLinkDown"` | `"onIdle"`
...
- `nodeId`: stable logical node identity for a connection tree entry. Prefer this when you need something that survives reconnects.
- `connectionId`: the current transport connection identity surfaced by readiness events such as `onNodeReady()`. It may change after a reconnect.
- `sessionId`: the currently attached terminal session identity. It can be `null` before a terminal is attached or when no terminal is focused.
...
**`onIdle(handler: (conn: ConnectionSnapshot) => void): Disposable`**

Fires when a still-live connection releases all terminal references and transitions to `idle`.

```javascript
ctx.events.onIdle(conn => {
  console.log(`Idle: ${conn.label}`);
});
```

**`onNodeReady(handler: (info: { nodeId: string; connectionId: string }) => void): Disposable`**

Fires when a node reaches the ready state and plugin capabilities such as SFTP or IDE integration can safely attach.

**`onNodeDisconnected(handler: (info: { nodeId: string }) => void): Disposable`**

Fires when a node leaves the ready state and becomes disconnected.
...
If `sessionId` is `null`, treat the event as not being bound to an active terminal session. In that case, prefer returning the original input unchanged unless your plugin explicitly handles the unfocused case.
...
Apply the same rule to output processors: when `sessionId` is `null`, prefer pass-through behavior unless your processor is intentionally session-agnostic.
...
For shortcuts, `sessionId === null` usually means there is no focused remote terminal. Return `false` or `void` unless your shortcut is designed to work without a bound session.
...
A named icon map is exposed via `window.__OXIDE__.lucideIcons`, and the full icon module is available through `window.__OXIDE__.lucideReact` with a fallback proxy for missing PascalCase icons.

```javascript
const { lucideIcons, lucideReact, React } = window.__OXIDE__;
...
Icon naming: PascalCase (e.g. `Server`, `LayoutDashboard`, `GitBranch`). Use `lucideIcons['Name']` when indexing by string, and prefer `lucideReact.Name` when you want automatic fallback behavior. Check https://lucide.dev/icons/ for the full list.
    ...
    "contributes": {
      ...
      "properties": {
        ...
        "connectionHooks": {
          ...
          "items": {
            "type": "string",
            "enum": ["onConnect", "onDisconnect", "onReconnect", "onLinkDown", "onIdle"]
          }
        },
        "apiCommands": {
          "type": "array",
...
> This document covers the current Plugin API v3 surface shipped by OxideTerm as of 2026-04-09.
