...
**Method 2: Install from the registry (Recommended)**

1. Switch to the **Browse** tab in Plugin Manager
2. Search for or browse available plugins
3. Click **Install**
4. The plugin will be downloaded, verified, and installed automatically
5. After installation completes, the plugin is activated automatically

**Method 3: Update an installed plugin**

1. In the **Browse** tab, installed plugins with available updates show an **Update** button
2. Click **Update**
3. The old version will be uninstalled, and the new version will be installed and activated automatically
...
Copy `plugin-api.d.ts` from the OxideTerm repository root into your plugin project.
...
### Q: How do I publish a plugin to the official registry?

1. **Package the plugin** by zipping the plugin directory:
...
3. **Host the ZIP file** at a publicly accessible URL such as GitHub Releases or a CDN.

4. **Submit it to the registry**:
- for the official registry, open a PR to the OxideTerm repository and add your plugin entry
- for a private registry, add an entry to your own `registry.json`
...
### Q: How do I use a custom plugin registry?

Edit `~/.oxideterm/plugin-config.json`:
...
> Recommended: use the root-level `plugin-api.d.ts` file directly. It is a standalone, zero-dependency type definition file that you can copy into your plugin project for IntelliSense. See [FAQ: Can plugins use TypeScript?](#q-can-plugins-use-typescript)
...
| `src-tauri/src/commands/plugin_registry.rs` | Plugin registry lookup and installation |
