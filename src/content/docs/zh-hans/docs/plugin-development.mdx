...
> 适用于当前 OxideTerm 版本（Plugin API v3）
...
:::caution[安全提示]
当前插件运行在同一 JS 上下文中，理论上可以直接 `import '@tauri-apps/api/core'` 绕过 API 白名单。白名单是**纵深防御**措施，防止意外误用；真正的沙箱隔离需要 iframe/WebWorker 架构（未来计划）。**请仅安装来源可信的插件**。
:::
...
可选值：`"onConnect"` | `"onDisconnect"` | `"onReconnect"` | `"onLinkDown"` | `"onIdle"`
...
:::tip[提示]
大多数 SFTP 和端口转发操作可以直接通过 `ctx.sftp` 和 `ctx.forward` 命名空间调用，无需在 `apiCommands` 中声明。只有这两个命名空间未覆盖的底层命令才需要通过 `ctx.api.invoke()` 调用。
:::
...
`PluginContext` 是传递给 `activate(ctx)` 的唯一参数。它是一个深度冻结的对象，共有 19 个顶层字段，其中包括 `pluginId` 和 18 个 API 命名空间。v3 新增了 7 个只读命名空间。
...
```typescript
events.onIdle(handler: (snapshot: ConnectionSnapshot) => void): Disposable
```

当一个仍然存活的连接释放掉所有终端引用并进入 `'idle'` 状态时触发。

#### `onNodeReady(handler)`

```typescript
events.onNodeReady(handler: (info: { nodeId: string; connectionId: string }) => void): Disposable
```

当节点进入 ready 状态、插件可安全使用其能力（如 SFTP / IDE）时触发。

#### `onNodeDisconnected(handler)`

```typescript
events.onNodeDisconnected(handler: (info: { nodeId: string }) => void): Disposable
```

当节点离开 ready 状态并断开时触发。
...
:::caution
未在 manifest 中声明的 tabId 会抛出 `Error: Tab "xxx" not declared in plugin manifest contributes.tabs`
:::
...
:::caution
**不要使用硬编码颜色**（如 `text-white`、`bg-gray-800`）。始终使用语义化类名以兼容所有主题。
:::
...
```javascript
ctx.events.onNodeReady(({ nodeId, connectionId }) => {
  console.log(`Node ${nodeId} is ready on connection ${connectionId}`);
});

ctx.events.onNodeDisconnected(({ nodeId }) => {
...
- `nodeId`：稳定的逻辑节点标识，跨重连仍然可用，适合用于终端、SFTP、IDE 相关能力绑定。
- `connectionId`：当前传输层连接 ID，可能在重连后发生变化。
- `sessionId`：终端会话 ID；当节点还没有绑定终端，或者当前没有聚焦终端时，可以为 `null`。
...
补充说明：`ConnectionSnapshot.id` 表示当前连接实例 ID；如果你需要跨重连保持稳定的标识，请使用 `onNodeReady()` / `onNodeDisconnected()` 事件里的 `nodeId`。
...
:::caution[建议性白名单（Advisory Whitelist）]
...
3. **按需使用命名空间**：v3 提供 18 个 API 命名空间，外加顶层 `pluginId` 字段。如果你只需要 `ui` 和 `terminal`，不必关心 `profiler` 或 `ai`。
  ...
  "properties": {
    ...
    "contributes": {
      ...
      "properties": {
        ...
        "connectionHooks": {
          "type": "array",
          "items": { "type": "string", "enum": ["onConnect", "onDisconnect", "onReconnect", "onLinkDown", "onIdle"] }
        },
        "apiCommands": {
          "type": "array",
          "items": { "type": "string" }
