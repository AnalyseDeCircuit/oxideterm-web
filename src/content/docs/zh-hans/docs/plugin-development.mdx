...
{/* source-sha256: bf204dbf1339c1827ac161f537a72712f182e39d45d259102a05b91cfa98bb08 */}
...
- [4. Manifest 完全参考](#4-manifest-完全参考)
  ...
  - [4.8 contributes.aiTools](#48-contributesaitools)
  - [4.9 contributes.apiCommands](#49-contributesapicommands)
  - [4.10 locales](#410-locales)
- [5. 插件生命周期](#5-插件生命周期)
...
| **AI 工具声明** | `contributes.aiTools` | 可选 Tool Protocol v2 元数据，用于描述插件工具语义；不是执行权限边界 |
...
声明插件提供或计划提供给 OxideSens 使用的 AI 工具元数据。这个字段是 **Tool Protocol v2 的可选声明层**：旧插件可以完全不写；新插件写了以后，宿主可以在工具列表、权限提示、工具调用 UI 和未来的插件工具执行层中更准确地展示 capability、risk、target 和结构化结果。

OxideSens 现在内部采用 target-first 的任务编排器。内置聊天默认不会把所有底层宿主工具直接暴露给模型，插件工具也不会因为声明了 `aiTools` 就自动进入默认工具面；只有用户明确调用插件 participant，或宿主启用对应能力路径时，插件工具才会被纳入可见工具集合。manifest API 保持兼容；`aiTools` 是元数据，不是执行权限。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | `string` | 插件内工具名。未来暴露给 OxideSens 时会由宿主加插件命名空间，避免与内置工具冲突。 |
| `description` | `string` | 面向模型和用户的简短说明。 |
| `parameters` | `object?` | Function Calling 参数 JSON Schema。省略时按空对象处理。 |
| `capabilities` | `string[]?` | 语义能力列表，例如 `filesystem.read`、`terminal.send`、`state.list`。 |
| `risk` | `string?` | 明确风险等级。省略时宿主按 legacy 规则推断。 |
| `targetKinds` | `string[]?` | 可操作目标类型，例如 `ssh-node`、`terminal-session`、`sftp-session`。 |
| `resultSchema` | `object?` | 工具返回 envelope 中 `data` 字段的 JSON Schema。 |
...
- 未声明 `aiTools` 的插件继续按 legacy 插件处理。
- 声明 `aiTools` 不会自动授予任何额外权限，也不会绕过用户批准。
- `risk` / `capabilities` 只影响展示、审批提示和未来插件工具注册的默认语义；真正的宿主操作仍受 `PluginContext` API 与 `apiCommands` 白名单约束。
- 插件工具返回旧字符串或旧对象时，宿主会按 legacy result 显示；如果返回 Tool Protocol v2 envelope，工具 UI 会优先展示 `summary`、`data`、`warnings` 和 `meta.targetId`。
...
`ctx.events` 当前只公开以上 4 个连接生命周期事件。额外的节点/会话创建、关闭或空闲状态事件目前**不属于公开插件 API**。

如果你需要跟踪节点或会话树变化，请使用 `ctx.sessions`：
...
如需感知终端节点的增减或状态变化，请基于 `ctx.sessions` 暴露的树快照和节点状态订阅实现，而不是依赖未公开的内部事件名。
...
export type PluginToolCapability =
  | 'command.run' | 'terminal.send' | 'terminal.observe' | 'terminal.wait'
  | 'filesystem.read' | 'filesystem.write' | 'filesystem.search'
  | 'navigation.open' | 'state.list' | 'network.forward'
  | 'settings.read' | 'settings.write' | 'plugin.invoke' | 'mcp.invoke';

export type PluginToolTargetKind =
  | 'local-shell' | 'ssh-node' | 'terminal-session' | 'sftp-session'
  | 'ide-workspace' | 'app-tab' | 'mcp-server' | 'rag-index';

export type PluginToolRisk =
  | 'read' | 'write-file' | 'execute-command' | 'interactive-input'
  | 'destructive' | 'network-expose' | 'settings-change' | 'credential-sensitive';
...
export type PluginManifest = {
  ...
  contributes?: {
    ...
    /* tabs, sidebarPanels, settings, terminalHooks, terminalTransports,
       connectionHooks, apiCommands */
  };
  locales?: string;
};
  ...
  "properties": {
    ...
    "contributes": {
      ...
      "properties": {
        ...
        "aiTools": {
          "type": "array",
          "description": "Optional Tool Protocol v2 metadata for plugin AI tools",
          "items": {
            "type": "object",
...
*本文档基于当前仓库中的 OxideTerm Plugin API v3 源码更新。最后更新：2026-04-26。如有疑问，请参考上述源码文件或提交 Issue。*
