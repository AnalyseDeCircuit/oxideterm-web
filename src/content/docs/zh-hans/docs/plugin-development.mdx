...
根据稳定的 `nodeId` 反查当前连接快照。若节点不存在，或当前未绑定到连接，则返回 `null`。
...
其中，`onTreeChange()` 适合感知节点增删或树结构变化；`onNodeStateChange()` 适合在你已经持有某个 `nodeId` 时，订阅该节点的连接状态变化。
...
当外部同步流程在宿主常规变更链路之外修改了保存连接、保存转发或设置后，主动触发宿主刷新对应状态。
...
这个快照刻意只暴露非 secret 元数据。插件拿不到 password，也拿不到 key / certificate 的 passphrase；但会拿到 key_path、cert_path 和 proxy_chain 拓扑，方便同步类或审计类插件识别证书认证跳板，而不需要接触私密凭据。
...
如需感知终端节点的增减或连接状态变化，请基于 `ctx.sessions` 暴露的树快照和节点状态订阅实现，而不是依赖未公开的内部事件名。
