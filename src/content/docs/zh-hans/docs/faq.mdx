...
OxideTerm 是一款基于 Tauri 2.0（Rust）和 React 19（TypeScript）构建的 AI 原生 SSH 工作区。它将 SSH 终端、本地 PTY、SFTP、IDE 模式、端口转发、AI 辅助、插件系统、31+ 主题和 11 种语言整合在一个原生应用中。

### OxideTerm 免费吗？

OxideTerm 采用 **GPL-3.0**（GNU 通用公共许可证 v3.0）：
- **所有人**：免费——个人、教育、研究和商业用途
- **衍生作品**：必须在同一 GPL-3.0 许可证下发布
- **源代码**：分发修改版本时必须提供源代码
...
271,000+ 行 Rust 和 TypeScript 代码。
...
- 实时文件监控
- 43 个 JSON-RPC 方法
- 在 x86_64/aarch64 Linux 上自动部署；其他架构需自行从源码构建并手动上传
...
密码存储在操作系统原生钥匙串中（macOS 的钥匙串访问、Windows 凭据管理器、Linux 的 Secret Service）。**绝不** 以明文形式存储在配置文件中。

已保存连接的元数据也会在本地静态加密。OxideTerm 将真正的秘密保存在系统钥匙串中，同时使用同一层钥匙串保护的本地密钥去解锁磁盘上的连接配置。

有。插件运行在多层安全机制下：
- 键盘交互 / 2FA 在直连场景支持；通过 ProxyJump 链路显式使用 KeyboardInteractive 目前仍有限制

### 插件可以访问文件系统吗？

当远端工具需要使用本地 Agent 时，也可以按连接开启 Agent Forwarding。

### macOS 上应用无法启动

### 有官方插件吗？

有。OxideTerm 当前的官方插件主要提供可选工作流，而不是强制账号体系：

- **Cloud Sync** — 通过 WebDAV、HTTP JSON、Dropbox、Git 或 S3 对 `.oxide` 快照进行加密的自托管同步
- **Quick Commands** — 保存、组织并按主机筛选常用终端命令，一键执行

插件是完全按需启用的。核心应用仍然是本地优先，不要求任何云账号。
