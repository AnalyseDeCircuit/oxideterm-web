---
title: 简介
description: OxideTerm — 基于纯 Rust 和现代 Web 技术构建的 AI 原生 SSH 工作区。
---

import { Image } from 'astro:assets';

**OxideTerm** 是面向终端、文件、端口与远程上下文的 AI 原生 SSH 工作区。基于 **Tauri 2.0**（Rust 后端）和 **React 19**（TypeScript 前端）构建，生成 25–40 MB 的轻量级原生二进制文件，零 Electron、零 OpenSSL、零遥测，并使用纯 Rust SSH。
...
| 凭据保存在明文配置里 | **静态加密**：密码和 API Key 留在系统钥匙串，已保存连接元数据在本地密封加密 |
| 云依赖、必须注册账号的工具 | **本地优先**：默认零账号。Cloud Sync 也是通过官方插件按需启用 |
...
| 类别 | 功能 |
|------|------|
| **终端** | 本地 PTY（zsh/bash/fish/pwsh/WSL2）、SSH 远程、分屏、广播输入、会话录制/回放（asciicast v2）、自适应渲染（120 Hz+ 加速 / 60 Hz 正常 / 1–15 Hz 空闲）、命令面板（`⌘K`）、禅模式、31+ 主题 + 自定义编辑器 |
| **SSH 与认证** | 连接池与复用、ProxyJump（无限跳数）+ 拓扑图、优雅期自动重连、Agent Forwarding。认证：密码、SSH 密钥（RSA/Ed25519/ECDSA）、SSH Agent、证书、直连时的键盘交互 2FA、Known Hosts TOFU。ProxyJump 链路当前更适合使用密码/密钥/Agent/证书认证 |
...
| **插件** | 运行时 ESM 加载，18 个 API 命名空间，24 个 UI Kit 组件，冻结 API + Proxy ACL，熔断器，错误自动禁用。官方插件包括 Cloud Sync、Quick Commands 和 Telnet Client |
| **CLI** | `oxt` 伴侣工具：JSON-RPC 2.0，Unix Socket / Named Pipe |
| **安全** | .oxide 加密导出（ChaCha20-Poly1305 + Argon2id 256 MB）、OS 钥匙串、本地配置静态加密、Touch ID（macOS）、主机密钥 TOFU、`zeroize` 内存清除 |
...
- [连接管理](/zh-hans/docs/features/connections/) — 认证方式、ProxyJump、SSH config 导入、加密预设
- [插件系统](/zh-hans/docs/features/plugins/) — 官方插件与运行时扩展模型
